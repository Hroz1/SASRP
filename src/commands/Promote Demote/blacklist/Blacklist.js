...
module.exports = class RoleCommand extends BaseCommand {
  ...
  async run(client, message, args) {
    ...
    const Blacklist = message.guild.roles.cache.get('855956532486996038');
    ...
    if (!Blacklist) return message.channel.send('There is no Blacklist role to add.');
    ...
    await mentionedMember.roles.add(Blacklist.id).catch(err => message.channel.send(`I was unable to add the Blacklist role due to an error: ${err}`));
    ...
    const movementEmbed = new Discord.MessageEmbed()
      ...
      .setDescription(`<@${mentionedMember.user.id}> Blacklisted.`)
      .setColor("#000000")
      .setTimestamp();
