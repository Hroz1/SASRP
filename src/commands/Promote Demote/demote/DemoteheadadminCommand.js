...
module.exports = class RoleCommand extends BaseCommand {
  ...
  async run(client, message, args) {
    ...
    if (!headAdmin) return message.channel.send('There is no Administrator role to add.');
    ...
    await mentionedMember.roles.add(Admin.id).catch(err => message.channel.send(`I was unable to remove the Administrator role due to an error: ${err}`));
    ...
    const movementEmbed = new Discord.MessageEmbed()
      ...
      .setDescription(`<@${mentionedMember.user.id}> Demoted to Administrator.`)
      .setColor("#ab0808")
      .setTimestamp();
